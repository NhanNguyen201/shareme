...
export default function CreatePin({user}) {
    ...
    const savePin = () => {
        const {isValid, errors} = validatePin({title, asset: imageAsset?._id})
        if(isValid){
            const doc = {
                _type: 'pin',
                title,
    ...
    return (
        <div className="flex flex-col justify-center items-center mt-5 lg:h-4/5">
            {!pinValid && (
                Object.keys(pinErrors).map((pinError) => <p className="text-red-500 mb-5 text-xl transition-all duration-150 ease-in" key={pinError}>
                   {pinErrors[pinError]}
                </p>)
            )}
